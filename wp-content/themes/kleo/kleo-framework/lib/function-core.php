...
/*
 * Exclude from search added by FZ 17/7/2016
 */
	...
	function kleo_title()
	{
		...
		elseif (is_search())
		{
			...
			if(!empty($wp_query->found_posts))
			{
				...
				{
					$output =  $wp_query->found_posts ." ". __('Suchresultate für:','kleo_framework')." ".esc_attr( get_search_query() );
				}
				else
				{
					$output =  $wp_query->found_posts ." ". __('Suchresultate für:','kleo_framework')." ".esc_attr( get_search_query() );
			...
			else
			{
				if(!empty($_GET['s']))
				{
					$output = __('Suchresultate für:','kleo_framework')." ".esc_attr( get_search_query() );
				}
				else
				{
					$output = __('Bitte einen richtigen Suchbegriff eingeben','kleo_framework');
				}
			}
        ...
        elseif ( is_front_page() && !is_home() ) {
            ...
            $output = __('Fehler 404 - Seite nicht gefunden','kleo_framework');
		}
		else {
