...
function namespace_add_custom_types( $query ) {
  if( is_category() || is_tag() && empty( $query->query_vars['suppress_filters'] ) ) {
    $query->set( 'post_type', array(
     'post', 'nav_menu_item', 'fotoabzug', 'fotobuch', 'kalender', 'wanddeko', 'fotokarte', 'fotogeschenk'
		));
	  return $query;
	}
}
...
function kleo_post_listing( $atts ) {
	ob_start();
    ...
    if ( have_posts() ) :
    	echo '<div class="posts-listing-container"><div class="posts-listing small-listing">';
...
add_shortcode( 'post_listing', 'kleo_post_listing' );
...
function kleo_widget_categories_alt( $category_description, $category ) {
	return $category->name;
}
add_filter( 'category_description', 'kleo_widget_categories_alt', 10, 2 );

function tax_terms_lists_func( $atts ) {
	...
	if(!empty($terms)) { ?>
		<div class="posts-listing">
			...
			foreach($terms as $term) {
				if (function_exists('z_taxonomy_image_url')) {
					$taxonomy_image = z_taxonomy_image_url( $term->term_id, NULL, TRUE );
				}
				if(empty($taxonomy_image)) {
					$taxonomy_image = get_bloginfo('template_url').'/assets/img/placeholder.png';
				}
				$term_link = types_render_termmeta('outbound-link', array( 'term_id' => $term->term_id, 'raw' => true) );
				if(empty($term_link)) {
					$term_link = get_term_link($term);
				}
				$company_logo = types_render_termmeta("company-logo", array( "term_id" => $term->term_id, "alt" => "Company Logo", 'raw' => true ) );
				?>
				<article id="post-<?php the_ID(); ?>" <?php post_class(array("post-item")); ?>>
					<div class="row post-content animated animate-when-almost-visible el-appear">

						<div style="padding-left:10px;" class="col-xs-3 no-padding">
							<div class="post-image">
								<img style="width:124px" height="auto" alt="" src="<?php echo $taxonomy_image; ?>">
							</div>
							<?php if(!empty($company_logo)) { ?>
								<span class="ifolor_img">
									<img width="50" height="auto" src="<?php echo $company_logo; ?>">
								</span>
							<?php } ?>
						</div>
						...
						<div class="col-xs-9">
							...
							<div class="post-header">
								...
								<span style="font-size:21px;" class="post-item-title"><?php echo $term->name; ?></span><br/>
								<span style="font-size:14px;margin-left:1px;margin-bottom:4px;" class="post-item-info"><?php echo types_render_termmeta("short-description", array( "term_id" => $term->term_id)); ?></span>

							</div><!--end post-header-->

							<div class="post-footer"> 

								<span style="font-size:18px;margin-left:15px;float:right" class="butt-righ"> <b><a href="<?php echo $term_link; ?>" class="zurWebsiteSingle" rel="nofollow" title="ifolor Fotoabzüge"> Weiter</a></b></span>
		...
		_e("No taxonomy founds.", 'kleo_framework');
	}
	
    return $html;
}
add_shortcode( 'tax_terms_lists', 'tax_terms_lists_func' );
